...
async function saveCompanionInteraction(toolType, prompt, response, metadata = {}) {
    try {
        ...
        const docRef = await addDoc(collection(db, 'companionChats'), companionData);
        console.log('Companion interaction saved:', docRef.id);
        return docRef.id;
    } catch (error) {
        console.error('Error saving companion interaction:', error);
        return null;
    }
}
...
Format your response clearly with sections.`;
    ...
    if (aiResponse) {
        ...
        await saveCompanionInteraction('assignment-helper', prompt, aiResponse, metadata);
            ...
            <div class="bg-white dark:bg-gray-700 p-6 rounded-lg border border-gray-200 dark:border-gray-600">
                ...
                <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-600">
                    <p class="text-sm text-green-600 dark:text-green-400">
                        ...
                        Saved to your chats for future reference
                    </p>
                </div>
            </div>
        `;
...
Make it educational, well-structured, and relevant to Ghanaian students following the GES curriculum.`;
    ...
    if (aiResponse) {
        ...
        await saveCompanionInteraction('content-writer', prompt, aiResponse, metadata);
...
4. Key takeaways`;
    ...
    if (aiResponse) {
        ...
        const metadata = {
            ...
            concept,
            level,
            levelDescription: levelMap[level]
        };
        await saveCompanionInteraction('explainer', prompt, aiResponse, metadata);
...
Make it clear, engaging, and educational.`;
    ...
    if (aiResponse) {
        ...
        await saveCompanionInteraction('reference-topic', prompt, aiResponse, metadata);
        ...
        explanationDiv.innerHTML = `
            <div class="bg-red-100 dark:bg-red-900 p-4 rounded-lg border border-red-300 dark:border-red-700">
                <p class="text-red-800 dark:text-red-200">
                    <i class="fas fa-exclamation-circle mr-2"></i>
                    AI explanation not available. Please configure your OpenAI API key in companion.js
                </p>
            </div>
        `;
    }
